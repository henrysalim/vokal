...
export default function CekSuaraScreen() {
  ...
  const startScan = async () => {
    try {
      ...
      setTimeout(async () => {
        ...
        const score = geminiResult
          ? Math.round(geminiResult.score * 0.65 + dsp.urgencyScore * 0.35)
          : dsp.urgencyScore;
        const status =
          score >= 60 ? "bahaya" : score >= 30 ? "waspada" : "aman";
        const statusText =
          status === "bahaya"
            ? "Kloning AI terdeteksi"
            : status === "waspada"
              ? "Mencurigakan"
              : "Normal/Aman";
        import("../../utils/analysisHistory").then(({ addAnalysisLog }) => {
          addAnalysisLog(
            "suara",
            "Cek Suara: Rekaman Audio Baru",
            `Risiko Kloning AI: ${score}% (${statusText}).`,
            status,
          );
        });
        setScanState("result");
  ...
  const finalScore = geminiResult
    ? Math.round(
        geminiResult.score * 0.65 + (dspResult?.urgencyScore ?? 0) * 0.35,
      )
  ...
  return (
    <SafeAreaView edges={["top"]} className="flex-1 bg-cream">
      ...
      >
        ...
        <View className="mb-6">
          <View className="flex-row items-center justify-between mb-1">
            <AppText size="2xl" className="font-heading text-espresso">
        ...
        <View className="items-center mb-8 mt-2">
          <View className="relative items-center justify-center h-44 w-44">
            {scanState === "recording" && (
              <>
                <Animated.View
                  className="absolute w-40 h-40 rounded-full border border-terracotta/50 bg-terracotta/15"
                  style={animatedPulse}
                />
                <Animated.View
                  className="absolute w-56 h-56 rounded-full border border-terracotta/20 bg-terracotta/5"
                  style={animatedPulse}
                />
              </>
            )}
          ...
          <View className="mt-6 items-center min-h-[64px]">
            ...
            {scanState === "recording" && (
              <Animated.View entering={FadeIn} className="items-center">
                ...
                >
                  Merekam... {recordingSeconds}/5 detik
                </AppText>
            ...
            {scanState === "analyzing_gemini" && (
              <Animated.View entering={FadeIn} className="items-center">
                ...
                <AppText
                  size="xs"
                  className="text-text-muted font-body text-center"
                >
                  Interpretasi AI mendalam berdasarkan fitur DSP
                </AppText>
              </Animated.View>
            )}
          </View>
        ...
        {scanState === "result" && dspResult && (
          <Animated.View entering={FadeInDown.springify()}>
            ...
            {isHighRisk ? (
              <View className="w-full bg-terracotta/10 rounded-[24px] p-5 border-2 border-terracotta/30 mb-5">
                <View className="items-center mb-4 bg-white/60 p-4 rounded-2xl">
                  <AppText size="3xl" className="font-display text-terracotta">
                    {finalScore}%
                  </AppText>
                  <AppText
                    size="xs"
                    className="text-terracotta font-heading mt-1 uppercase tracking-wider"
                  >
                    Risiko Kloning AI
                  </AppText>
                </View>
                <View className="flex-row items-center gap-3 mb-3">
                  ...
                  <View className="flex-1">
                    <AppText
                      size="base"
                      className="text-terracotta font-heading leading-tight"
                    >
                      Pola Suara Sintetis Terdeteksi
                    </AppText>
                    <AppText
                      size="xs"
                      className="text-espresso/70 font-body mt-0.5"
                    >
                      Suara menunjukkan anomali kloning AI.
                    </AppText>
                  </View>
                </View>
              ...
              <View className="w-full bg-olive/10 rounded-[24px] p-5 border-2 border-olive/30 mb-5">
                <View className="items-center mb-4 bg-white/60 p-4 rounded-2xl">
                  <AppText size="3xl" className="font-display text-olive">
                    {(100 - finalScore).toFixed(0)}%
                  </AppText>
                  <AppText
                    size="xs"
                    className="text-olive font-heading mt-1 uppercase tracking-wider"
                  >
                    Kewajaran Vokal Manusia
                  </AppText>
                </View>
                <View className="flex-row items-center gap-3 mb-3">
                  ...
                  <View className="flex-1">
                    <AppText
                      size="base"
                      className="text-olive font-heading leading-tight"
                    >
                      Pola Bicara Natural
                    </AppText>
                    <AppText
                      ...
                      className="text-espresso/70 font-body mt-0.5"
                    >
                      Tidak ada indikasi kloning AI yang signifikan.
                    </AppText>
                  </View>
                </View>
              ...
              <View className="bg-surface rounded-2xl p-4 border border-terracotta/15 mb-4">
                <View className="flex-row items-center gap-2 mb-2">
                  ...
                  <AppText size="sm" className="font-heading text-espresso">
                    Interpretasi Gemini AI
                  </AppText>
                  <View className="bg-terracotta/10 px-2 py-0.5 rounded-full ml-auto">
                    <AppText size="xs" className="text-terracotta font-bold">
                ...
                <AppText
                  size="xs"
                  className="font-body text-espresso/70 leading-relaxed"
                >
                ...
                {geminiResult.flags.map((flag) => (
                  ...
                  >
                    ...
                    <AppText
                      size="xs"
                      className="font-body text-espresso/70 flex-1 leading-relaxed"
                    >
                      {flag.description}
                    </AppText>
                  </View>
            ...
            <AppText size="sm" className="text-espresso font-heading mb-2">
              Analisis DSP Lokal (5 Fitur):
            </AppText>
            <AppText
              size="xs"
              className="text-text-muted font-body mb-3 leading-relaxed"
            >
              Tap fitur untuk lihat penjelasan • Merah=mencurigakan ·
              Hijau=alami
            </AppText>
            ...
            <View className="bg-espresso/5 border border-espresso/10 rounded-2xl p-4 flex-row items-center gap-3 mb-5">
              ...
              <View className="flex-1">
                <AppText size="xs" className="font-heading text-espresso">
                  DSP: {dspResult.urgencyScore}%{" "}
                  {geminiResult ? `· Gemini: ${geminiResult.score}%` : ""} ·
                  Final: {finalScore}%
                </AppText>
                <AppText size="xs" className="font-body text-text-muted mt-0.5">
                  {dspResult.silenceGaps} jeda · {dspResult.peaks} puncak · Avg:{" "}
                  {dspResult.avgVolume} dB
                </AppText>
              </View>
            </View>
      ...
      <Modal visible={showPanicMode} animationType="slide" statusBarTranslucent>
        <View style={{ flex: 1, backgroundColor: "#1C2529" }}>
          <LinearGradient
            ...
            style={{ flex: 1 }}
            className="justify-between px-6 pt-24 pb-12"
          >
