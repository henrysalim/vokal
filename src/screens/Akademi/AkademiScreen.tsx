    ...
    <SafeAreaView edges={['top']} className="flex-1 bg-cream">
      ...
      <View className="flex-row justify-between items-center px-5 pt-4 pb-2 z-10 bg-cream">
        <View className="flex-row items-center bg-mustard/20 px-3 py-2 rounded-full border border-mustard/30">
          ...
          <Text className="font-heading text-mustard text-sm">{MOCK_USER.xp} XP</Text>
        </View>
          ...
          <Animated.View entering={FadeInDown.springify()} className="px-5 mb-8">
            <View className="bg-surface rounded-[24px] p-6 shadow-sm border border-espresso/5">
              <View className="items-center mb-6 mt-2">
                <Text className="text-5xl font-display text-espresso">{xp.toLocaleString()}</Text>
                <Text className="text-text-muted text-xs font-body uppercase tracking-widest mt-1">Total XP Kamu</Text>
              </View>
              
              <Text className="text-espresso font-heading text-lg text-center mb-2">🎓 Selamat Datang di Akademi!</Text>
              <Text className="text-text-muted font-body text-[13px] text-center mb-8 leading-relaxed">
              ...
              >
                <Text className="font-heading text-espresso text-base">Mulai Perjalanan</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        ) : (
        ...
        <Animated.View entering={FadeInDown.delay(800).springify()} className="px-5">
          <View className="bg-espresso rounded-[24px] p-5 overflow-hidden shadow-md">
            ...
            <View className="flex-row items-center gap-2 mb-4">
              ...
              <Text className="text-white font-heading text-base">Papan Peringkat RT/RW</Text>
            </View>
            ...
            <View className="gap-3">
              {leaderboard.map((item, i) => (
                <View key={item.name} className={`flex-row items-center p-3 rounded-2xl ${item.isMe ? 'bg-mustard border border-mustard/50' : 'bg-surface/10'}`}>
                  <Text className={`font-display text-sm w-6 text-center ${item.isMe ? 'text-espresso' : 'text-cream/50'}`}>#{item.rank}</Text>
                  <Text className={`flex-1 font-heading text-sm ml-2 ${item.isMe ? 'text-espresso' : 'text-cream'}`}>{item.name}</Text>
                  <Text className={`font-display text-xs ${item.isMe ? 'text-espresso' : 'text-mustard'}`}>{item.score.toLocaleString()} XP</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View className="flex-1 justify-end bg-espresso/40">
          ...
          <Animated.View entering={SlideInDown.springify()} exiting={SlideOutDown} className="bg-cream rounded-t-[32px] p-6 pt-8 border-t border-espresso/10">
            <View className="items-center mb-6">
              ...
              <Text className="font-heading text-2xl text-espresso text-center">{selectedModule?.title}</Text>
              <Text className="font-body text-text-muted text-center mt-2 px-4">{selectedModule?.desc}</Text>
              
              <View className="flex-row items-center bg-mustard/20 px-4 py-2 rounded-full mt-4">
                <Text className="font-heading text-mustard text-sm">Ganjaran: +{selectedModule?.xp} XP</Text>
              </View>
            </View>
            ...
            >
              <Text className="font-heading text-espresso text-lg">Mulai Latihan</Text>
            </TouchableOpacity>
          </Animated.View>
        </View>
      </Modal>
      ...
      <Modal visible={showQuiz} transparent animationType="fade">
        <View className="flex-1 bg-espresso/90 justify-center items-center px-5">
          <Animated.View entering={ZoomIn.springify()} exiting={ZoomOut} className="bg-cream w-full rounded-[32px] overflow-hidden shadow-xl border border-espresso/10">
            <View className="p-6">
              <View className="flex-row justify-between items-center mb-6">
                <Text className="text-espresso font-heading text-lg">Quiz VOKAL</Text>
              ...
              <Text className="text-text-muted font-body text-sm mb-4 text-center">Dengarkan klip suara berikut:</Text>
              ...
              <View className="bg-surface rounded-3xl p-5 items-center mb-6 border border-espresso/5 shadow-sm">
                ...
                <Text className="text-espresso font-display text-xs">"Ma, ini Rini, minta transfer cepat 2 juta..."</Text>
              </View>

              <Text className="text-espresso font-heading text-center mb-4">Apakah ini Suara Asli atau AI Kloningan?</Text>
              ...
              <View className="flex-row gap-3">
                <TouchableOpacity disabled={quizFinished} onPress={() => handleAnswer('asli')} className={`flex-1 py-4 rounded-2xl items-center border-2 ${quizAnswer === 'asli' ? 'bg-terracotta border-terracotta' : 'bg-surface border-espresso/10'}`}>
                  <Text className={`font-display text-sm ${quizAnswer === 'asli' ? 'text-white' : 'text-espresso'}`}>Suara Asli</Text>
                </TouchableOpacity>
                <TouchableOpacity disabled={quizFinished} onPress={() => handleAnswer('ai')} className={`flex-1 py-4 rounded-2xl items-center border-2 ${quizAnswer === 'ai' ? 'bg-olive border-olive' : 'bg-surface border-espresso/10'}`}>
                  <Text className={`font-display text-sm ${quizAnswer === 'ai' ? 'text-white' : 'text-espresso'}`}>AI Kloningan</Text>
                </TouchableOpacity>
              </View>
              ...
              {quizFinished && (
                <Animated.View style={animatedScale} className={`mt-6 p-4 rounded-2xl flex-row items-start gap-3 ${quizAnswer === 'ai' ? 'bg-olive/10 border-olive/20' : 'bg-terracotta/10 border-terracotta/20'} border`}>
                  ...
                  <View className="flex-1">
                    <Text className={`font-heading text-sm mb-1 ${quizAnswer === 'ai' ? 'text-olive' : 'text-terracotta'}`}>
                      {quizAnswer === 'ai' ? 'Tepat Sekali! (+20 XP)' : lives === 0 ? 'Game Over! Nyawa Habis!' : 'Oops, Hampir Benar! (-1 Nyawa)'}
                    </Text>
                    <Text className="font-body text-[11px] text-text-muted leading-relaxed">
          ...
          <View className="bg-cream rounded-t-[32px] p-6 pt-8 max-h-[70%]">
            <View className="flex-row justify-between items-center mb-6">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="font-heading text-xl text-espresso">Papan Peringkat</Text>
              </View>
              ...
              <View className="gap-3 pb-8">
                {leaderboard.map((item, i) => (
                  <View key={item.name} className={`flex-row items-center p-4 rounded-2xl ${item.isMe ? 'bg-mustard/20 border border-mustard/50' : 'bg-surface border border-espresso/5'}`}>
                    <Text className={`font-display text-sm w-6 text-center ${item.isMe ? 'text-espresso' : 'text-espresso/40'}`}>#{item.rank}</Text>
                    <Text className={`flex-1 font-heading text-sm ml-2 ${item.isMe ? 'text-espresso' : 'text-espresso/70'}`}>{item.name}</Text>
                    <Text className={`font-display text-xs ${item.isMe ? 'text-espresso' : 'text-mustard'}`}>{item.score.toLocaleString()} XP</Text>
                  </View>
                ))}
              </View>
