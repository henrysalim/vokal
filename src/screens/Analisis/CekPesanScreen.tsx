  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      import("../../utils/analysisHistory").then(({ addAnalysisLog }) => {
        addAnalysisLog(
          "pesan",
          "Cek Pesan: Teks Percakapan",
          `Gemini AI: ${analysis.verdict} (Skor Risiko: ${analysis.score}/100).`,
          status,
        );
      });
    } catch (err: any) {
      ...
      <SafeAreaView edges={["top"]} className="flex-1 bg-cream">
        ...
        >
          ...
          <View className="flex-row items-center justify-between mb-5">
            <View>
              ...
              <View className="flex-row items-center gap-1.5 mt-0.5">
                ...
                <AppText size="xs" className="text-olive font-body">
          ...
          <View className="bg-espresso/5 rounded-xl px-4 py-3 mb-4 border border-espresso/10">
            <AppText
              size="xs"
              className="text-text-muted font-display uppercase tracking-widest mb-1"
            >
              Teks yang Dianalisis
            </AppText>
            <AppText
              size="xs"
              className="text-espresso/70 font-body leading-relaxed"
              numberOfLines={3}
            >
              {inputText}
            </AppText>
          ...
          {result.flags.length === 0 && result.score < 20 && (
            <View className="bg-olive/10 rounded-2xl p-4 flex-row gap-3 items-center border border-olive/20">
              ...
              <View className="flex-1">
                <AppText size="sm" className="text-olive font-heading">
                  Tidak Ada Tanda Bahaya
                </AppText>
                <AppText size="xs" className="text-olive/70 font-body mt-0.5">
                  Pesan ini tampak aman. Tidak ada pola penipuan yang
                  terdeteksi.
                </AppText>
              </View>
            </View>
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-cream">
      ...
      >
        ...
        <Animated.View entering={FadeInDown.springify()} className="mb-5">
          ...
          >
            Tempel teks WA, SMS, atau chat yang mencurigakan. Gemini AI akan
            menganalisis 8+ pola penipuan Indonesia.
          </AppText>
        </Animated.View>
        ...
        <Animated.View
          ...
          className="flex-row items-center gap-2 bg-olive/10 border border-olive/20 rounded-xl px-3 py-2.5 mb-5"
        >
        ...
        <Animated.View entering={FadeInDown.delay(100).springify()}>
          ...
          <AppText size="xs" className="text-text-muted font-body mt-2 mb-4">
            ...
            {inputText.length < 15 && inputText.length > 0 ? "(min. 15)" : ""}
          </AppText>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(150).springify()}>
          <TouchableOpacity
            ...
            style={{
              backgroundColor:
                inputText.trim().length < 15 ? "#9E8E7E" : "#74822F",
            }}
            onPress={handleAnalyze}
