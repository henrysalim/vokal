// Supabase Edge Function: vokal-analyze
// Deploy: supabase functions deploy vokal-analyze --no-verify-jwt
...
function buildTextPrompt(text: string, analysisType: string): string {
  const ctx = analysisType === 'email' ? 'email yang diterima pengguna' : 'pesan WhatsApp, SMS, atau chat';
...
serve(async (req) => {
  ...
  // Jika diakses lewat browser (GET), tampilkan status check
  if (req.method === 'GET') {
  ...
  try {
    ...
    const result = safeParse(rawText) ?? {
      ...
      flags: [{ id: 'err', label: 'Analisis Gagal', description: 'Coba lagi atau verifikasi secara manual.', level: 'warning' }],
    };
